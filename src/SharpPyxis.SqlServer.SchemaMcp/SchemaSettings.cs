...
/// </summary>
internal sealed record SchemaSettings(string ConnectionString, string DatabaseName, string? Purpose)
{
    /// <summary>
    /// Reads <c>CONNECTION_STRING</c>, required, and <c>DDL_DESCRIPTION</c>, optional. The database
    /// is taken from the connection string, which must therefore name one.
    /// </summary>
    public static SchemaSettings FromEnvironment()
    {
        ...
        return new SchemaSettings(connectionString, databaseName, purpose);
    }
